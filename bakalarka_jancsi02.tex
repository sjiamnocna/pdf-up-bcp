...
class dataLoader extends Endpoints{
	...
	\section{Postup implementace}
        ...
        \subsubsection{Generátor}
            ...
            Potom je~možné provést nahrazení proměnných, vložením \emph{tokenů} z~proměnné na~místo určení. Pokud je~závislá proměnná definována dříve než~ta~kterou chceme nahradit, jde o~chybu. Pokud závislá proměnná není zpracovaná, zpracuji ji~voláním téže funkce (\emph{rekurze}). Tím~zajistím, že~proměnné budou nahrazeny ve~správném pořadí. Pokud se~proměnná nenahradí, znamená to, že~neexistuje, což je~také chyba.

            V~matematickém režimu je~potřeba~vyčíslit hodnotu výrazu zapsanou v~\emph{tokenech}, např.~\texttt{randInt(0,42) + 1} pro~náhodné liché číslo. To~realizuju pomocí algoritmu podobnému tzv.~\textbf{Postfixové notaci} (\textbf{Reverzní polská notace}), která nevyužívá závorky ani~prioritu operátorů a~je~vhodná ke~zpracování počítačem. Výraz je~zapsán jako~posloupnost \emph{operandů} a~\emph{operátorů}, např.~$19 + ( 26 + 1 )$ je~převeden do~\emph{postfixového} zápisu jako $19 26 1 + +$. Dvojice \emph{operandů} (\emph{hodnot}) jsou zpracovávány operátorem a~tedy jednodušší pro~zpracování pomocí funkce (např.\,\texttt{sum(a, b)}) v~programovacím jazyce. Pro~názornost algoritmu vyčíslení je~možné využít \href{https://www.free-online-calculator-use.com/postfix-evaluator.html#}{vizualizaci}. \cite{postfixnotation}
	...
	\section{Závěr}
        ...
        V~práci jsem se~snažil používat aktuální technologie a~postupy, používat základní poučky pro vývoj aplikací a~psát čitelný, dokumentovaný a~rozšiřitelný kód. Informační technologie se~ale neustále vyvíjí a~je~nutné sledovat a~vyhodnocovat nové trendy a~technologie, protože v~době této publikace již~mohou být~některé postupy překonané. Většina principů ale zůstane platná a~je~možné na~nich~stavět.
        ...
        Základ pro~tvorbu aplikace pomocí jazyka \emph{Go} a~knihovny \emph{Vite} je~k~použití pro~vytváření projektů na~\emph{Gitlabu} \href{https://gitlab.com/sjiamnocna/gormrest}{\texttt{sjiamnocna/gormrest}}. Dokumentace ke~kódu je~v~komentářích, dokumentačních řetězcích a~souborech \textbf{README} v~popisu repozitářů.
