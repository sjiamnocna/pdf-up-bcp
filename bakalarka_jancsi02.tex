...
UPDATE users SET email = 'mujmejl@seznam.cz\', role=\'admin', WHERE id = 42'
	...
	\chapter{Návrh aplikace}
        ...
        \section{Výběr technologií}
            ...
            Pro~\textbf{časově kritické aplikace} operující v~reálném čase, např.~řízení auta, kde~by \textbf{režijní zátěž} z~provozu \acrshort{GC} mohla způsobit opožděné spuštění jiných důležitých procesů, je~vhodné zvolit jazyk s~menší \emph{režijní zátěží}, např.~\textbf{Rust} nebo~\emph{Zig}.

            Pro~aplikace jako je~webový server, je~\emph{Go}~dobrá volba. \emph{Go} navíc automaticky formátuje kód, což~zjednodušuje práci s~ním a~umožňuje lepší spolupráci v~týmu, jelikož není~nutné hledat jednotný \textbf{codestyle}, který by~nejlépe vyhovoval potřebám vývoje.
...
class dataLoader extends Endpoints{
    ...
    public function actionEndpoint{
        ...
        return JSONResponse({
            ...
            \textbf{Next.js} mi~jedoduše umožní využít výhody~\acrshort{SSR} aplikace. \emph{Next.js} nabízí širokou škálu funkcí, které usnadňují vývoj moderních aplikací, včetně~např.:
