...
UPDATE users SET email = 'mujmejl@seznam.cz\', role=\'admin', WHERE id = 42'
	...
	\section{Návrh aplikace}
        ...
        \subsection{Výběr technologií}
            ...
            Zároveň během vývoje není třeba celý program kompilovat, stačí jej~spustit příkazem \texttt{go run} a~aplikace funguje. Při~publikaci programu potom stačí program sestavit pomocí \texttt{go build} a~\emph{Go} vytvoří optimalizovaný spustitelný binární soubor.
            
            \emph{Go}~je~jazyk, který je~velice oblíbený pro~tvorbu webových aplikací. Umožňuje při~vývoji jednoduše importovat existující balíčky z~GIT repozitářů, nebo jiných zdrojů přímo z~internetu, a~používat je~v~aplikaci a~sám si~tyto balíčky stáhne a~nainstaluje. Tím~značně urychluje vývoj celé aplikace.
            
            Pro~aplikace jako je~webový server, je~\emph{Go}~dobrá volba.
...
class dataLoader extends Endpoints{
	...
	\section{Postup implementace}
        ...
        Při~vývojí ve~\emph{Vite} využiju nastavení \emph{proxy}, čímž~požadavky, jejichž \emph{query} začíná \texttt{/api/\dots} přesměruju na~kontejner s~\emph{backendem} aplikace.
        
        \textbf{Vývojová verze} bude obsahovat kontejnery pro~\emph{React}, \emph{Go} a~\emph{databázi}. \emph{Go}~kontejner bude~obsahovat nástroj \emph{Air}, který umožní automatické \emph{sestavení} a~spuštění aplikace při~změně zdrojového kódu. \emph{Frontend} bude spuštěný ve~vývojové verzi \emph{Vite}, která již~podobnou funkci (\textbf{Hot-reload}) obsahuje.
        ...
        Každý kontejner bude mít vlastní adresář. \emph{Backend} bude v~adresáři \texttt{goapp} a~\emph{frontend} v~adresáři \texttt{vite}. Všechny kontejnery budou mít své~úložiště v~adresáři \texttt{data}, kam budou ukládat trvalá data, aby~se~zachovala při~restartu kontejneru. To~zjednoduší zálohování dat a~případnou migraci na~jiný server.
        ...
        \subsection{Backend}
            ...
            Samotný \emph{backend} bude tvořen dvěma částmi, aby~byl~lépe testovatelný a~rozšiřitelný. Půjde o~\emph{webový server}, který bude přijímat požadavky a~spravovat uživatelská data, a~\emph{generátor}, který přijme požadavky na~příklad a~vygeneruje jeho části podle zadání. Ty~potom předá ve~strukturované podobě výslednému zobrazení (např.~\emph{PDF soubor} nebo~jakékoliv jiné \emph{zobrazení}, které s~daty bude umět pracovat). V~případě nalezených chyb (např. nekompatibilní nastavení, problém v~závislostech proměnných, jejich nesprávný zápis\dots), které by~mohly vést k~nevytvoření příkladu, sdělí chyby uživateli, případně se~pokusí, pokud je~to~možné, samostatně chyby odstranit.
            ...
            V~případě, že~uživatel zapomene heslo, je~potřeba mu~umožnit jej~změnit. K~tomu je~potřeba mu~poslat e-mail s~odkazem na~stránku, kde~si~může heslo změnit. Tento odkaz musí být unikátní a~platný pouze po~omezenou dobu, aby~nebylo možné jej~zneužít. K~tomu využiji jednoznačný \emph{token}, obsahující zahešované informace o~daném požadavku, platný pouze po~omezenou dobu. Zároveň v~databázi vytvořím záznam s~informací o~tom, že~uživatel požádal o~změnu hesla. Po~přístupu na~stránku pomocí odkazu zkontroluji, zda~je~požadavek platný a~pokud ano, heslo změním.

            Přihlašovací údaje budu tzv.~\textbf{cachovat} (\textbf{kešovat}), což~znamená uložit si~data do~paměti~\emph{RAM} běžícího programu pro~pozdější využití, aby byla rychleji k~použití bez~nutnosti přístupu k~databázi nebo~provádění náročných výpočtů, které by~stejně vedly ke~stejným výsledkům.
        ...
        \subsubsection{Funkce Init}
            ...
            Spouštíme také migraci databáze, aby \emph{GORM} vytvořil databázové tabulky nebo je~aktualizoval. Jednotlivé modely dat jsou vidět ve~zdrojovém kódu aplikace.

            Další úlohou pro~funkci \emph{init} bude~příprava šablon. Jelikož budu používat balíčky \texttt{html/template} pro~zpracování šablon s~proměnnými, je~v~rámci výkonu lepší šablony předpřipravit, aby~se~nemusely při~každém požadavku znovu překládat. Při~použití potom pouze vyvolám připravenou šablonu a~poskytnu jí~data, která potřebuje do~šablony doplnit. To~zajistí rychlejší odezvu aplikace. Šablony používám zejména pro~vytváření e-mailů, které budou odesílány uživatelům, a případných chybových hlášek podle jejich jazykových preferencí.
        ...
        \subsubsection{Generátor}
            ...
            \emph{Parser} musí rozložit zápisy hodnot proměnných na~jednotlivé \emph{dílčí řetězce} (\emph{tokeny}) a~identifikovat jejich typ. K~tomu použiju modul \href{github.com/bzick/tokenizer}{\texttt{bzick/tokenizer}}. Ten~ve~vstupním textu identifikuje základní typy jako~--~čísla, identifikátory, ap. a vyfiltruje tzv.~\textbf{bílé znaky} (mezery, ukončení řádku~ap.). Nakonec~vrátí seznam \emph{tokenů}, které v~textu našel. Podle kontextu (typů okolních \emph{tokenů}) je~bude~potřeba sloučit a~určit, jaký význam v~aplikaci zastávají a~jak je~potřeba je~zpracovat. Písmeno za~číslem bude polynom, identifikátor se~znakem \texttt{\$} je~použitá proměnná, závorky za proměnnou je~funkce s~argumenty~atd.
            
            Jako první musí proběhnout spuštění funkcí ve~všech proměnných. Tím~jsou vygenerovány náhodné nebo~vypočítané hodnoty.
            
            Proměnné budou označeny za~\textbf{kompletní} pokud nebudou obsahovat žádné funkce ani~proměnné. Ty~není třeba dále~zpracovávat. Zbytek bude zpracován postupně, dokud nebudou všechny proměnné kompletní.

            Poté je možné provést nahrazení proměnných, spojením \emph{tokenů} do~jednoho řetězce a~nahrazením proměnných jejich hodnotami. Tuto akci je nutné opakovat vícekrát, dokud nejsou vyřízeny všechny závislosti na~proměnných.
            Pokud je~závislá proměnná definována dříve než~ta~zdrojová, vložíme ji~na~seznam odloženého zpracování. Pokud ani~po~vyhodnocení všech zadaných proměnných není~seznam odložených proměnných prázdný, jde~o~chybu zadání a~je~potřeba informovat uživatele a~přerušit generování příkladu.

            Pro~vyčíslení koeficientů jednotlivých členů výrazu (pokud to uživatel u~proměnné nezakáže) nejdříve dosadím hodnoty existujících proměnných a~poté pomocí tzv.~\textbf{Postfixové notace} (Někdy také \textbf{Reverzní polská notace}), která se~k~vyčíslování výrazů pomocí počítačů běžně používá, vyčíslím koeficienty u~případných mnohočlenů. \cite{postfixnotation}

            Protože pracuju s~jazykem \emph
            {Go}, obdrží \emph{generátor} data ve~formě \emph{struktury} definující příklad a~výsledkem je~struktura obsahující zpracované zadání a~případně vygenerované možnosti s~nahrazenými a~vyčíslenými hodnotami, aby bylo možné hodnoty předat zobrazení (generátoru \emph{PDF} nebo \emph{JSON} pro~použití na~webu). Výsledná data budou obsahovat pouze zadání otázky a~možnosti odpovědí. Na~místě zobrazení uživatelského vstupu bude vložen \emph{placeholder}, který bude zpracován až~podle~volby~zobrazení.
            
            \emph{Generátor} samotný nemá přístup k~databázi, takže uložení vygenerovaných hodnot zajistí webserver, hodnoty si~načte ze~struktury obdržené z~generátoru. Potom konkrétní hodnoty uloží do~databáze vygenerovaných příkladů s~jednoznačným identifikátorem.

        \subsubsection{Autentizace uživatelů}
            Registraci a~přihlášení do~aplikace bude možné více způsoby. Pro~všechny způsoby přihlášení bude existovat~jedna tabulka s~tajnými přihlašovacími údaji, jejíž záznamy se~budou odkazovat na~konkrétního uživatele v~tabulce s~uživateli. Uživatelé se~budou moci přihlásit pomocí \emph{e-mailové adresy a~hesla}, ale také pomocí \emph{Google} účtu. V~budoucnu tak~bude~možné přidat více služeb k~ověření identity uživatele.
            
            Na~\emph{Frontendu} použiju nástroj \emph{Firebase} od~firmy \emph{Google}, který umožňuje mmj.~jednoduchou \emph{autentizaci} uživatelů pomocí různých webových služeb. Po~přihlášení dostane každý takový uživatel svůj unikátní \textbf{UID} (\textbf{User ID})
            
            Tyto údaje budu ukládat do~tabulky \texttt{users} a~při~každém přihlášení je~budu kontrolovat. V~případě, že~uživatel neexistuje, vytvořím ho~a~přiřadím mu~jeho \emph{UID}. Tyto údaje poslouží k~přihlášení podobně jako jméno a~heslo, které v~tabulce bude také.
            
            Pokud~bude~ověření úspěšné, uživatel je~přihlášen a~vložím \emph{hash} aktuální relace do~\emph{JWT} i~do~databáze. Při~každém přístupu bude~zkontrolována platnost daného připojení. To~umožní mít~přehled o~přihlášených uživatelích a~zabezpečí přístup k~datům. Umožní to~i~vzdálené odhlášení. Jakékoliv akce s~účtem budou zapsány do~tabulky akcí a~k~dispozici uživateli, který získá lepší přehled o~přihlašování a~dění na~účtu a~možnost navrácení účtu do~minulého stavu.

        \subsection{Frontend}
            \emph{Frontend} bude vytvořen pomocí nástroje \emph{Vite} v~jazyce \emph{Typescript} s~použitím knihovny \emph{React}. Pro~komunikaci s~backendem využiju dříve napsaný \emph{Typescript} modul \href{https://gitlab.com/sjiamnocna/renette-api}{\emph{sjiamnocna/renette-api}}, který je~napsaný k~udržování \emph{JWT} v~klientské části aplikace a~tedy~zjednoduší komunikaci \emph{frontendu} s~\emph{backendem}. Při~každé důležité změně, jako přihlášení nebo~odhlášení uživatele se~změní \emph{JWT token}, čímž se~snažím redukovat riziko jeho zneužití.

            \emph{Frontend} bude založen na~\emph{Vite} v~\emph{Reactu}.
            Její základ bude v~anglickém jazyce, do~ostatních jazyků (vč.~Češtiny) budu aplikaci překládat. K~tomu slouží \emph{Node} modul \texttt{i18n}. Překlady budou uloženy v~adresáři \texttt{locales} a~budou se~načítat dynamicky podle aktuálního jazyka. V~případě, že~překlad chybí, zobrazí se~anglický text. Překlady budou vytvořeny pomocí \emph{Google Translate} a~budou přeloženy ručně.
            ...
            \textbf{UI (User Interface)}~--~uživatelské rozhrání bude co~nejjednoduší, k~tvorbě využiju knihovnu \textbf{Semantic UI}, která nabízí jednoduché a~přizpůsobitelné komponenty. \emph{Semantic UI} je~dostupná jako \emph{npm} balíček, který je~možné nainstalovat pomocí \emph{npm} nebo~\emph{yarn}. Použití externí \emph{CSS knihovny} zkrátí práci na~\emph{UI}, umožňuje použít \emph{komponenty}, které jsou připraveny na~různé velikosti zařízení a~v~základu přizpůsobené pravidlům o~přístupnosti webové aplikace.
        ...
        \subsection{Testování}
            Nemá význam testovat celou aplikaci. \emph{GIN} i~\emph{GORM} jsou aktivně testovány při~jejich vývoji, navíc při~sebemenší chybě aplikace vůbec nenastartuje a~pokud něco selže při~tvorbě \emph{Docker} kontejneru, aplikace se~vůbec nespustí. A~chyby na~straně externích služeb (\emph{SMTP}) jsou~zachyceny v~\emph{Sentry}. Postupem času budu testovací scénáře přidávat podle přibývajícího kódu a~nalezených chyb.
            
            Proto se~budu věnovat testování pouze u~\emph{generátoru} (\texttt{\emph{Go} test}) a~\emph{frontendu} (\texttt{Jest}), případně menším funkcím, které nejsou závislé na~dalších částech aplikace a~měly by~generovat pro~kombinaci stejných dat stejný výsledek (viz.~\emph{Pure function}).

            Testovány budou všechny scénáře, které jsou od~\emph{generátoru} požadovány. Testy budou spouštěny samostatně během vývoje i~jako~součást budování \emph{produkčního} \emph{docker kontejneru}.

            Na~\emph{frontendu} budu pomocí \emph{jest} testovat funkčnost \emph{komponent} a~jejich správné vykreslení. V~případě, že~se~chyba vyskytne za~běhu aplikace, zaznamená ji~\emph{Sentry} a~upozorní na~nutnost opravy.

	\section{Závěr}
        \subsection{Budoucí rozvoj aplikace}
            ...
            To~by~umožnilo nechat si~vygenerovat testy přímo do~prostředí \textbf{Moodlu}, nebo~je~rychle sdílet přes \textbf{MS~Teams}. Zajímavá by~mohla~být i~integrace do~\textbf{CMS} systémů jako~např.~\textbf{Wordpress} nebo~\textbf{Drupal}.

            Učitelům by~mohla být~umožněna tvorba kampaní, kde by~žáci postupovali v~rámci kampaně a~otevírali nové schopnosti. Třeba sérii cvičení na~přijímací zkoušky na~konkrétní střední školu, nebo~soutěžní příklady nebo by~mohli sledovat pokrok žáků v~právě probíraném učivu.
            ...
            Aplikace by~se~dala využít také~k~tvorbě interaktivních výukových materiálů s~minikvízy, které by~mohly být využity pro~výuku na~dálku. Učitel by~mohl vytvořit výukový materiál, který by~žákům poslal a~ti~by~si~ho~mohli procházet. Součástí materiálů by~byly kvízové otázky, které slouží k~procvičování učiva. Učitel by~pak~mohl získat zpětnou vazbu, jak si~žáci vedou a~které části učiva jim dělají největší problémy. K~rychlému zobrazení řešení by~pomohla možnost otevření příkladu v~aplikaci jako je~\emph{Wolfram Alpha}.
            
            Důležitou součástí materiálů jsou zdroje, které učitel použil při~tvorbě materiálu, aby~si~žáci mohli přečíst více o~daném tématu a~ověřit jejich správnost. Webové zdroje bude aplikace pravidelně kontrolovat a~v~případě změny informuje autora materiálů, který bude moci informace aktualizovat. To~umožní vytvářet výukové materiály, které budou udržovány aktuální.
            
            Pro~realizaci výše zmíněných rozšíření by~bylo vhodné získat sponzory, kteří by~přispěli na~vývoji aplikace, což~by~umožnilo věnovat se~vývoji naplno a~rychleji rozšiřovat její funkce. Do~budoucna by~bylo potřeba vývoj rozšířit o~další vývojáře, kteří by~se~podíleli na~vývoji a~rozšiřování aplikace. Možností je~také přidat možnost ověření uživatele pomocí ITIC/ISIC, čímž by~se~zvýšila jejich důvěryhodnost.
        ...
        Výsledná aplikace je veřejně dostupná na~adrese \href{https://pisemkomat.cz}{\texttt{pisemkomat.cz}}. Kód některých částí této práce je~možné veřejně najít na~\href{https://gitlab.com/pisemkomat}{Gitlabu}.

        Základ pro~tvorbu moderní aplikace s~překlady a~možností lokálního vývoje pomocí jazyka \emph{Go} a~\emph{Vite} je~volně k~dispozici pod~licencí \textbf{MIT} na~Gitlabu: \href{https://gitlab.com/sjiamnocna/gormrest}{\texttt{sjiamnocna/gormrest}}.
