...
\begin{document}
		...
		\begin{center}
            {
                ...
                \vspace{-5mm}
                \Large
                \normalfont{Katedra technické a~informační výchovy}
            }
    ...
    \begin{singletonpage}{Anotace / Annotation}
        Práce zdůrazňuje využití \acrdefprint[short]{IT}{Informační Technologie} ve~vzdělávání a~zaměřuje~se na~jejich využití ke~generování písemných testů. Zkoumá existující metody tvorby testů a~jejich náležitosti a~představuje novou webovou aplikaci, která umožňuje učitelům efektivně vytvářet testy z~úloh s~parametricky zadanými vlastnostmi. Aplikace podporuje uzavřené i~otevřené úlohy a~bude integrovatelná s~jinými systémy pomocí~\acrshort{API}.

        The~thesis emphasizes the~significance of~technology in~the~educational process and~contributes to~modernizing the~process of~preparing exams. Introduces the web application that allows users to~generate exams from~prepared questions with~parametrically defined properties. The~application will support both questions with options and~questions with~long~answers and will be integrable into other systems using~\acrshort{API}.
    \end{singletonpage}
    ...
    \chapter{Testy, testové úlohy}
        ...
        \section{Nástroje k tvorbě testů}
            ...
            Pokud je~potřeba procvičovat test k~přijímacím zkouškám, materiál je~možné nalézt na~oficiálních webech společností \emph{CERMAT} nebo~\emph{SCIO}. Existují také různě obsáhlé sbírky úloh a~testů, které jsou vydávány jako~samostatné knihy pro~různé obory a~obtížnost učiva.
    ...
    \chapter{Technologie a principy vývoje webových aplikací}
        ...
        Obvykle není nutné tvořit aplikaci od~začátku (tzv.~\textbf{na~zelené louce}). \uv{Pro~\acrshort{FE} i~\acrshort{BE} aplikace existuje na~webu velké množství použitelných materiálů. Ať~už~jsou to~různé grafické prvky nebo~serverové komponenty, to~vše je už~někde nejspíše hotové.}~\cite{itnetworkBestPractices}.
        ...
        \section{Základní statická HTML stránka}
            ...
            Jednotlivé prvky je~také možné označit pomocí \emph{identifikátorů} a~\emph{tříd}, které se~používají pro~jejich identifikaci ve~stránce. Zatímco \emph{tříd} může mít~prvek více a~jednu třídu je~možné použít vícekrát, týž~identifikátor může být~použit v~dokumentu pouze jednou~--~měl by~být unikátní. \emph{Třídy} se~používají pro~označení skupiny prvků, které mají stejný vzhled nebo~funkcionalitu. Prvek může mít~více \emph{tříd} a~kombinovat tak jejich vlastnosti~\cite{jpw:tridy}.
        ...
        \section{Single Page Application}
            \acrdefprint{SPA}{Single Page Application} je~aplikace, která je~prohlížečem načtena pouze jednou a~poté pouze aktualizuje její~obsah pomocí technologie \acrshort{AJAX}. Většinou k~práci s~daty využívá nějaké \acrshort{API}.
            ...
            \textbf{Serverless aplikace} využívají k~zajištění některých (nebo všech) svých~funkcí již~připravené \acrshort{API} služeb třetích stran, např.~\textbf{Google Firebase} nebo \textbf{Amazon Web Services}, což~usnadňuje vývoj a~umožňuje vývojářům soustředit~se na~vývoj aplikace, a~neřešit tolik správu serveru a~psaní \acrshort{BE}.
        ...
        \section{Kompilátor a interpret}
            ...
            \uv{\acrdefprint{JIT}{Just In Time} \textbf{kompilace} je způsob, jakým \emph{interprety} urychlují běh skriptů tím, že během provádění programu překládají často prováděné části kódu (tzv.~\textbf{hot paths}) do~strojového kódu. Tento proces začíná interpretací skriptu, identifikací \emph{hot~paths} a~jejich následným překladem do~optimalizovaného strojového kódu, který je~ukládán do~\emph{cache}. Díky tomu může v~příštím běhu programu interpret spouštět tyto části kódu přímo, což~významně urychluje jejich provádění a zvyšuje celkový výkon aplikace}~\cite{RamananJIT}.
        ...
        \section{Kompilátor}
            ...
            \begin{description}
                ...
                \item[Syntaktická analýza] kontroluje, jestli jsou jednotlivé \emph{tokeny} správně strukturovány a~v~jakém vztahu k~sobě stojí. Generuje \acrdefprint{AST}{Abstraktní Syntaktický Strom} (\textbf{Abstract Syntax Tree}), který reprezentuje strukturu programu a~umožňuje tak~další zpracování a~optimalizaci.
            ...
            \emph{Překladač} umožňuje k~optimalizaci používat výrazy, které jsou vyhodnoceny během překladu a~nezatěžují tolik aplikaci při~běhu. Může~jít o~různá \textbf{makra}, která jsou~provedena již~během překladu, nebo použití konstantních výrazů, které jsou v~kódu přímo nahrazeny. Některé jazyky (např.~Zig pomocí \texttt{comptime}) umožňují i~přímé vyhodnocení kódu během kompilace. 

            V~kontextu \textbf{kompilátorů} a~\textbf{optimalizace} kódu jsou tzv.~\textbf{bitové masky} a~další práce s~bitovou reprezentací čísel na~nejnižší úrovni výhodné. Použití instrukcí v~\acrdefprint{JSI}{Jazyce Strojových Instrukcí} (\acrdefprint{ASM}{Assembly}) pro~bitové maskování umožňuje vývojářům použít elementární instrukce procesoru pro~práci s~čísly (v~tomto případě převážně instrukce \texttt{AND}), což~vede ke~zvýšení výkonu a~efektivity programu.
            ...
            To~využívá např.~systém Linux při~určování práv k~souborům. Ukázka použití v~aplikaci pro generování testů je~vidět v~kódu~\ref{code:iota}.b.
...
func (u *User) UserCan(p Permissions) bool {
    ...
    return u.Permission&p == p
                ...
                \end{minipage}
                \caption{Příklady využití \emph{iota} číselníků a bitové masky}
                \label{code:iota}
            \end{code}
        ...
        \section{Web Assembly}
            Standard \acrdefprint{WASM}{Web Assembly}, zavedený v~roce 2017, mění způsob vývoje webových aplikací. Díky němu je~možné spouštět programy napsané v~jazycích jako~\textbf{C}, \textbf{C++}, \textbf{Rust}, nebo i~\textbf{Go} přímo v~prohlížeči, nezávisle na~platformě (Windows, MacOS, Linux ap.). Tím~překonává omezení \acrshort{JS} a~otevírá dveře novým možnostem k výkonným, bezpečným a~jednoduše přenositelným aplikacím~\cite{MDN:WASM}.
            
            To~umožňuje pomocí \acrshort{WASM} provozovat náročnější webové aplikací, jako jsou náročnější hry, 3D~modelování, komplexní nástroje pro~úpravu fotografií, nebo decentralizované aplikace (DApps) běžící na~technologii \textbf{blockchain}. \acrshort{WASM} je~tak~klíčovou technologií pro~budoucnost webového vývoje~\cite{WASM:photoshop}.

        \section{Procedurální a~objektově orientované programování}
            \textbf{Procedurální programování} je~způsob programování, kdy se~program skládá z~jednotlivých \textbf{procedur}, které se~volají po~sobě (\textbf{sekvenčně}, \textbf{synchronně}). \emph{Procedura} je~část programu, která vykonává určitou činnost. \emph{Procedury} mohou mít~parametry, které jsou jim~předány při~volání a~mění jejich funkci. \emph{Procedury} mohou volat další \emph{procedury}, což~umožňuje znovupoužití kódu. \emph{Funkce} jsou~podobné \emph{procedurám} s~tím rozdílem, že~funkce má~tzv.~\textbf{návratovou hodnotu}, kterou po~svém dokončení vrací klíčovým slovem (\texttt{return}). \emph{Procedura} je~část kódu, která se~vkládá do~běhu programu a~pouze vykoná své~instrukce, nemá \emph{návratovou hodnotu}, může mít pouze \emph{vedlejší efekt}.
            ...
            \acrdefprint{OOP}{Objektově Orientované Programování} je~programovací paradigma, které se~snaží modelovat objekty z~reálného světa. V~\emph{OOP} se~program skládá z~objektů, které mají své~vlastnosti a~metody (funkce). \emph{Objekty} vznikají ze~tříd. Třída je~předpis, podle kterého je~vytvořen (\emph{inicializován}) konkrétní~objekt.
...
const App = () => {
    return (
        <h1 className="greeting">Hello World!</h1>
            ...
            \emph{Vite} (i~\emph{CRA}) zrychluje spouštění aplikace a~zjednodušuje práci díky tzv.~\emph{hot reload}, což~znamená, že~se~při~každé změně kódu \acrshort{FE} aplikace v~prohlížeči automaticky načte znovu s~novými změnami, bez~nutnosti manipulace s~prohlížečem~\cite{vitejs}.
        ...
        \section{Client-Server}
            ...
            \textbf{Klient} je~aplikace na~zařízení uživatele (webový prohlížeč), který služby \emph{serveru} využívá. V~případě webové aplikace je~to~webový prohlížeč, který zobrazuje \acrshort{HTML} stránku. V~případě mobilní aplikace je to~aplikace, která se~spouští na~mobilním zařízení a~komunikuje se~serverem. Např.~messengery, sociální sítě~\cite{ClientServer}.

        \section{Webová služba}
            Webová služba je~program, který běží na~\emph{serveru} a~poskytuje data \emph{klientovi}. Na \emph{serveru} může běžet více \emph{webových služeb}, rozlišují se \emph{portem}, na~kterém běží a~přijímají instrukce a~data. Např.~webová služba \acrdefprint{HTTP}{Hypertext Transfer Protocol}, která poskytuje webová data a~soubory, poběží na~\emph{portu~80}, resp.~v~dnešní době zejména~\emph{443} (\acrdefprint{HTTPS}{\acrshort{HTTP} Secure}). Další služby mohou být např.~emailová služba \acrdefprint{SMTP}{Simple Mail Transfer Protocol}.
        ...
        \section{Domain Name Server}
            \acrdefprint{DNS}{Domain Name Server}, někdy také \emph{Domain Name System}, je~systém, který překládá doménová jména na~\textbf{IP adresy}. To~umožňuje uživatelům používat lidem čitelná \emph{doménová jména} namísto \emph{IP~adres} cílového serveru, které jsou pro~uživatele těžko zapamatovatelné.
            ...
            \textbf{Doménové jméno} je~složeno ze~tří částí. První částí je~\emph{subdoména}, která je~volitelná. Druhou částí je~\emph{doména} (2.\,řádu) a~třetí část je~\emph{doména nejvyššího řádu} (\acrdefprint{TLD}{Top Level Domain}) a~může jít např.~o~národní koncovky (\texttt{.cz}, \texttt{.sk}, \ldots), některou ze~specifických domén (\texttt{.edu}, \texttt{.org}, \texttt{.app}, \ldots). Doména 2.~řádu a~\acrshort{TLD} jsou povinnou součástí adresy. Doménové jméno je~po~zadání do~prohlížeče v~procesu nazývaném \textbf{\acrshort{DNS} resolve} přeloženo na~\emph{IP adresu}, která se~dále použije k~odeslání požadavku~\cite{tld:mdn}.
        ...
        \section{Frontend, Backend}
            Aplikace lze~díky architektuře \acrshort{MVC} rozdělit na~\textbf{frontend} a~\textbf{backend}, které spolu komunikují a~plní funkce aplikace, ale~jinak zůstávají oddělené.
        ...
        \section{Application Interface}
            \acrdefprint{API}{Application Programming Interface} je~rozhraní, které umožňuje komunikaci mezi~aplikacemi. V~případě webových aplikací se~jedná o~rozhraní mezi \acrshort{FE} a~\acrshort{BE}. Některá \emph{rozhraní} jsou určena ke~komunikaci mezi aplikacemi nebo~dokonce s~\emph{hardwarem}.
            ...
            Velice oblíbená je~také~metoda \emph{gRPC} (\emph{Google Remote Procedure Call}), která je~založena na~protokolu \emph{HTTP/2} a~používá binární formát \emph{Protocol Buffers}. \emph{RPC} je~zkratka pro~\emph{Remote Procedure Call} a~funguje tak, že spouští vzdálené funkce, které jsou na~serveru.
        ...
        \section{Representational state transfer}
            ...
            Souhrnně se~těmto operacím říká \acrdefprint[short]{CRUD}{Create Read Update Delete} operace, což~je~zkratka pro~\textbf{Create}, \textbf{Read}, \textbf{Update} a~\textbf{Delete}.
        ...
        \section{Zabezpečení}
            ...
            \subsection{Autorizace}
                ...
                Je~potřeba zkontrolovat vlastnictví nebo~přidělená oprávnění (\uv{může uživatel článek jen zobrazit nebo i~upravovat?}). Např.~pokud je~uživatel autorem příkladu, může s~ním libovolně manipulovat. Pokud příklad vytvořil někdo jiný, musí k~úpravě dostat oprávnění~\cite{graham2021ethical}.
            ...
            \acrdefprint{SQL}{Structured Query Language} je~jazyk, používaný pro~manipulaci s~daty uloženými v~rámci relačních databází. \acrshort{SQL} je~deklarativní jazyk, což~znamená, že~dotaz popisuje, jaká data je~potřeba získat, a~ne jakým způsobem. \acrshort{SQL} je~standardizovaný jazyk, který je~podporován většinou relačních databází. V~jazyce~\acrshort{SQL} jsou~zapisovány tzv.~\textbf{SQL dotazy} (\textbf{SQL Query}), které mohou ukládat, načítat, upravovat, nebo~mazat data v~databázi~\cite{laurencik2018sql}.
...
UPDATE users SET email = 'mujmejl@seznam.cz\', role=\'admin', WHERE id = 42'
                ...
                Další kontroly vstupních dat mohou probíhat i~na~úrovni databáze, např.~při~indexaci. To~už ale~je na~uvážení autora konkrétní aplikace~\cite{w3s:SQLInjection, itnetwork:SQLInjection}.
        ...
        \section{42}
            Číslo \uv{\textbf{42}}~(\cite{adams1979hitchhikers}) někteří vývojáři používají jako~nenápadný odkaz na~zábavní rádiový pořad a~knihu Stopařův průvodce po~galaxii britského autora Douglase Adamse, vydávaný od~r.~1979. Jde o~způsob, jak~lidé z~\acrshort{IT} a~jiných oborů vyjadřují svůj smysl pro~humor~\cite{knuth1992art}.

            \emph{42}~je číslo, které v~příběhu dal velký počítač \textbf{Hlubina myšlení} (\textbf{Deep thought}) po~dlouhém počítání na~otázku "života, vesmíru a~vůbec", myšleno na~jakoukoliv otázku ve~vesmíru. Vtipně se tak vyskytuje v~různých podobách, jako všeříkající poznámka k~dané problematice. V~\acrshort{IT} např.~v~komentářích, dokumentacích, nebo je~přímo použito v~kódu.
            
            V~různých oborech má~však~\emph{42} reálný význam. Existují studie, zamýšlející~se nad~významem a~důkazy reálného významu tohoto čísla. Jde např.~o~jedno z~nejdéle řešených parametrů v~rozsahu od~0 do~100 \textbf{kubické diofantovské rovnice} $x^3 + y^3 + z^3 = k$~\cite{mit2019answerlife}, který počítače po~celém světě řešily přes~milion hodin. To~je~v~kontextu knihy zajímavé, jelikož v~knize superpočítač \emph{Hlubina myšlení} odpovídá na~otázku neúměrně dlouho, konkrétně \texttt{7,5\,milionu let}~, a~odpoví číslem \emph{42}~\cite{adams1979hitchhikers}.
            
            Další zajímavou souvislostí je, že~v~\acrdefprint[short]{ASCII}{American Standard Code for Information Interchange} tabulce (viz~\url{https://www.ascii-code.com/}), je~znak s~kódem~\emph{42} hvězdička (asterisk). V~různých implementacích nahrazuje jeden libovolný znak (tzv.~\textbf(wildcard)) nebo~v~\textbf{regulárních výrazech} jde o~tzv.~\textbf{kvantifikátor} označující libovolné množství z~určené množiny znaků.
            
            Číslo~\emph{42} je~také součástí několika významných algoritmů, např.~\textbf{SHA-1} nebo~\textbf{MD5}, které se~používají pro~\emph{hashování} hesel nebo~\emph{kontrolní součet} souborů. V~\emph{ASCII} tabulce je~\emph{42} znak \textbf{asterisk} (\textbf{*}), který se~používá jako~\emph{wildcard} v~\emph{regulárních výrazech} nebo~ve~\emph{fulltextovém} vyhledávání~\cite{regular_expressions, sha1, md5}.
        ...
        \section{Verzovací systém GIT}
            ...
            \subsection{Gitlab Actions}
                ...
                Během procesu je~sestaven \emph{kontejner}, ve~kterém se~spustí přednastavené testy a~poté je~aplikace sestavena. Pokud vše proběhne v~pořádku, aplikace je~automaticky nasazena na~produkční server. Použitím \emph{CI/CD} procesů je~zjednodušeno nasazování aplikace a~zrychluje se tak proces~vývoje~\cite{gitlab:actions}.
        ...
        \section{Integrated Development Environment a nástroje pro psaní kódu}
            ...
            Jazykové modely \emph{AI}, jako~\emph{ChatGPT} či~\emph{Google Bard} potom pomáhají zejména ke~konzultaci problémů a~získání nápadů, jak je~vyřešit. Svým způsobem tím~suplují síť webových fór \emph{Stack Overflow}, kde~si vývojáři navzájem radí s~řešením problémů, zejména z~oboru~\acrshort{IT}. Tyto \emph{AI} nástroje jsou skvělé na~opakované úkoly, jako přepisování struktur dat nebo hromadných příkazů.
        ...
        \section{Mezinárodní lokalizace}
            ...
            V~rámci \acrshort{BE} aplikace je~potřeba přeložit zejména e-mailové zprávy a~hlášky, které budou odesílány uživatelům. Na~\acrshort{FE} pak~celé \emph{uživatelské rozhraní (\acrshort{UI})}, které bude uživatel používat.
        ...
        \section{Unified Modeling Language}
            ...
            K~naplánování této~práce jsem vybral \textbf{diagram případů užití} (\textbf{Use Case Diagram}) a~\textbf{diagram~tříd} (\textbf{Class Diagram}) ze~standardu \emph{UML}.
            
            Je~důležité zmínit, že~neexistují předem dané postupy, jak diagramy tvořit a~co~vše do nich zahrnout, pouze standardizovaný způsob jejich struktury a~významů. Způsob, jakým vývojář funkcionalitu modeluje, je~čistě na~jeho osobním zvážení, dokud se~drží definice~\emph{UML}. Je~ale důležité, aby svá~rozhodnutí zdůvodnil a~průběžně dokumentoval~\cite{uml:FowlerBook, uml:diagram}.
	...
	\chapter{Návrh aplikace}
        ...
        \section{Návrh fungování aplikace}
        ...
        \section{Návrh struktury databáze}
            Pro~ukládání dat v~aplikaci použiji relační databázový systém \emph{MySQL}. Databáze je~soubor dat, která jsou~uspořádána tak, aby~byla snadno vyhledatelná a~měla nějaký význam.

            Při~plánování databáze využiji \emph{diagram tříd} k~odvození konkrétních modelů a~vztahů mezi~nimi. \emph{Use case diagram} zase pomůže zjistit, jaké \textbf{SQL~dotazy} (\textbf{SQL Query}) budou používány, což~umožní lepší promyšlení indexaci jednotlivých sloupců v~\acrdefprint[short]{DB}{Databáze}.
        ...
        \section{Výběr technologií}
            ...
            \LaTeX~je~profesionální nástroj pro~sazbu textů s~možnostmi matematických vzorců, který je~používán zejména v~akademickém prostředí~\cite{Rybicka2003:latex}. V~aplikaci využiji formát zápisu matematických výrazů z~\LaTeX{u}. \LaTeX~má~mnoho dalších využití a~byl~použit při~tvorbě písemné části této~práce.
...
type Question struct {
	...
	Parse bool
                    ...
                    \end{minted}
                    \subcaption{\emph{GORM} model (Go struktura)}
                \end{minipage}
                \hfill
                \begin{minipage}[t]{1\linewidth}
                    \begin{minted}{sql}
CREATE TABLE `questions` (
    ...
    CONSTRAINT `fk_questions_author` FOREIGN KEY (`author_id`) REFERENCES `users` (`user_id`)
                    ...
                    \end{minted}
                    \subcaption{Vytvořené tabulky s relacemi, omezeními a indexy}
                \end{minipage}
            ...
            \textbf{Alpine Linux} je~odlehčená distribuce \emph{Linuxu} zaměřená na~jednoduchost, zabezpečení a~efektivní využití prostoru na~disku, určená zejména pro~nasazení v~kontejnerech, kde~očekáváme malou zátěž na~úložiště a~zdroje. Jde~nicméně o~plnohodnotný systém, který je~možné použít i~na~serveru nebo~jiném stroji. Celková velikost distribuce v~kontejneru je~jen kolem 8\,MB (cca. 130\,MB v~plné verzi pro~počítač s~grafickým rozhraním), což~je~velice málo~\cite{alpinelinuxAbout}.
	...
	\chapter{Postup implementace}
        ...
        \subsection{Router}
            ...
            Při~každém volání zkontroluji a~do~požadavku načtu informace o~aktuálním uživateli. S~těmito daty pak~mohou pracovat další \emph{handlery}. Spuštění \emph{Gin} serveru (routeru), nastavení \emph{callbacků} a~vytvoření skupin \emph{endpointů} je~vidět v~kódu~\ref{go:enginerun}.

            Důležitou součástí aplikace \emph{GINu} je~tzv.~\textbf{Router}, který z~příchozího požadavku zjistí, který \emph{koncový bod} je~volán a~předá volání~příslušnému \emph{handleru}. \emph{Router} také umožňuje vytvářet skupiny \emph{koncových bodů}, které mají společný základ~\emph{URL} adresy a~které mohou sdílet část logiky. To~umožňuje vytvářet aplikace, které mají jednotnou strukturu a~jsou přehledné. \emph{Router} funguje podobně i~pro obsluhování požadavků na~\acrshort{FE}.
        ...
        \section{Frontend}
            ...
            \acrdefprint{UI}{User Interface} (\textbf{uživatelské rozhraní}) bude~vytvořeno s~použitím knihovny \acrdefprint{TW}{Tailwind CSS}. \acrshort{TW} je~\textbf{utility-first} knihovna. Soustředí~se na~poskytnutí jednoduchých prvků prostřednictvím \acrshort{CSS} tříd, které je~možné ke~komponentám přidávat. Skládáním těchto tříd je~možné vytvořit potřebný vzhled komponent. \acrshort{TW} je~také možné rozšířit o~vlastní třídy nebo~další knihovny, které rozšiřují jeho funkčnost a~mohou být~použity v~rámci celé aplikace. Pro~výslednou podobu aplikace potom \acrshort{TW} vytvoří \acrshort{CSS} kód pouze z~použitých tříd, čímž ještě omezí množství dat, které jsou přenášeny v~podobě stylů~\cite{Rappin2022ModernCSSTailwind}.

            Konkrétně využiji knihovnu komponent \textbf{Shadcn}, která poskytuje jednoduše použitelné a~upravitelné komponenty, které jsou optimalizovány pro~přístupnost i~\acrshort{SEO}, a~využívá \acrshort{TW}~\cite{logrocket:Shadcn}. K~tvorbě \acrshort{UI} je~ale samozřejmě možné využít i~jiné knihovny stylů (např.~\textbf{Bootstrap}) nebo~další různé přístupy ke~stylování.
    ...
    \chapter{Závěr}
        ...
        Kód generátoru je~přiložen k~práci formou \texttt{*.ZIP} archivu.
        ...
        Kód některých částí aplikace je~možné najít na~Gitlabu (\url{https://gitlab.com/pisemkomat}).

        Přes všechny snahy s~pluginy a~konfiguracemi pro~\acrshort{I18n} v~\emph{NextJS} zjistil, že~je~v~\emph{NextJS} již~od~r.~2021 nahlášena chyba (viz~\url{https://github.com/vercel/next.js/issues/21565}), která znemožňuje použití \emph{I18n} zároveň s~konfigurací \emph{proxy}. Z~tohoto důvodu bude~řešení problému překladů a~spuštění veřejné aplikace realizováno mimo~rámec této práce.

        Aplikace je~dostupná také pomocí~\acrshort{API} a~je možné ji~integrovat do~dalších prostředí formou pluginů. Uživatelé tak budou moci používat generované příklady např.~v~prostředí \textbf{Moodlu} nebo~\acrdefprint{CMS}{Content Management System} jako~např.~\textbf{Wordpress}. Dokumentace k~\acrshort{API} je~k~dispozici~ve~veřejné části webu.
