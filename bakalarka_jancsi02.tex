...
UPDATE users SET email = 'mujmejl@seznam.cz\', role=\'admin', WHERE id = 42'
        ...
        \section{42}
            Číslo \uv{\textbf{42}}~(\cite{adams1979hitchhikers}) někteří vývojáři používají jako~nenápadný odkaz na~zábavní rádiový pořad a~knihu Stopařův průvodce po~galaxií britského autora Douglese Adamse, vydávaný od~r.~1979. Jde o~způsob, jak~lidé z~\acrshort{IT} a~jiných oborů vyjadřují svůj smysl pro~humor~\cite{knuth1992art}.
...
class dataLoader extends Endpoints{
	...
	\chapter{Postup implementace}
        ...
        \section{Frontend}
            ...
            K~tvorbě \emph{UI} je~samozřejmě možné použít i~\textbf{frameworky} jako \textbf{Bootstrap}, \textbf{Foundation}, \textbf{Materialize~CSS} nebo~jiné přístupy ke~stylování.
