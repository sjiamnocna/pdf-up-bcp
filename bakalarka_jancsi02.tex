...
\begin{document}
    ...
    \begin{singletonpage}{Prohlášení}
        ...
        Pro~lepší pochopení kontextu byly~některé části práce konzultovány s~umělou inteligencí (ChatGPT, Bard), texty v~této práci jsou~však výsledkem samostatné práce.
...
UPDATE users SET email = 'mujmejl@seznam.cz\', role=\'admin', WHERE id = 42'
        ...
        \section{Testování}
            ...
            \subsection{Testování API}
                Pro~testování \acrshort{API} existují nástroje, které umožní vytvářet požadavky bez~nutnosti mít~dokončenu \emph{frontendovou} část. Mezi nejznámější patří \textbf{Postman} nebo~\textbf{Insomnia} atd.
                
                Tyto~nástroje navíc většinou nabízejí možnost vytvořit si~sadu automatických testů, které je~možné spouštět při~sestavování aplikace v~\emph{Dockeru} a~ověřit si~tak, že~\acrshort{API} stále funguje správně - např.~jsou dostupné všechny \emph{endpointy}.

                K~tomuto účelu použiji nástroj \emph{Insomnia}. S~nastaveným testovacím skriptem si~ukládám nový \emph{token} a~při~následujícím požadavku jej~posílám v~hlavičce požadavku. Tímto způsobem mohu~testovat \acrshort{BE}, bez~omezení bezpečnostních opatření.
	...
	\chapter{Návrh aplikace}
        ...
        \section{Výběr technologií}
            ...
            Pro~tvorbu \acrshort{FE} použiji knihovnu~\textbf{React}. Umožní mi~tvorbu efektivního \textbf{uživatelského rozhrání}~--~\acrdefprint{UI}{User Interface}.

            Při~vývoji aplikace v~Reactu použiji i~\acrshort{TS}. \emph{Facebook} s~použitím~\acrshort{TS} počítá a~vydává mimo~\emph{React} také~balíček s~příslušnými datovými typy. \acrshort{TS} podporuje i~\acrshort{JSX} pomocí souborů s~příponou \texttt{*.TSX}. \cite[Refeerence/Handbook/JSX]{TypeScript}
...
class dataLoader extends Endpoints{
    ...
    public function actionEndpoint{
        ...
        return JSONResponse({
            ...
            \textbf{Vite} je~nástroj zjednodušující tvorbu \emph{javascript frontendových aplikací} v~různých knihovnách. Je~velice rychlý a~jednoduchý na~použití. Bude použit pro~vytvoření \acrshort{FE} aplikace. Prostřednictvím \emph{proxy} serveru bude komunikovat s~\acrshort{BE}.
	...
	\chapter{Postup implementace}
        Jako první jsem~vytvořil základní adresářovou strukturu aplikace. Pro~lepší automatizaci a~konzistenci prostředí, kdekoliv aplikaci spustím budou \emph{vývojové} i~\emph{produkční} prostředí tvořit \emph{Docker kontejnery}.

        V~produkčním dojde k~přepnutí vývojového na~produkční prostředí a~dojde ke~spuštění testů a~následně \emph{kompilaci}. Spuštěná výsledná služba tak~bude~lépe optimalizována.
        
        Při~vývojí v~\emph{NextJS} využiji nastavení tzv.~\emph{proxy}. Požadavky, jejichž \emph{URI query} začíná \texttt{/api/\dots} nasměruje na~\acrshort{BE} aplikace.
        
        \textbf{Vývojová verze} budou kontejnery pro~\emph{React}, \emph{Go} a~\emph{databázi}. \emph{Go}~kontejner bude~obsahovat nástroj \emph{Air}, který umožní automatické \emph{sestavení} a~znovuspuštění aplikace při~změně zdrojového kódu. \acrshort{FE} bude spuštěný ve~vývojové verzi \emph{NextJS}, která již~má funkci (\textbf{Hot reload}), která při~změně automaticky přenačte stránku v~prohlížeči.
        ...
        Kód pro~\acrshort{BE} bude v~adresáři \texttt{goapp} a~kód pro~\acrshort{FE} v~adresáři \texttt{vite}. Všechny kontejnery budou mít své~úložiště v~adresáři \texttt{data}, kde budou trvalá data, která se~zachovají při~restartu kontejneru. To~zjednoduší zálohování dat a~případný přesun na~jiný server.
        ...
        \section{Testování}
            ...
            Na~\acrshort{FE} budu testovat funkčnost \emph{komponent}. V~případě, že~se~chyba vyskytne za~běhu aplikace, zaznamená ji~\emph{Sentry} a~upozorní na~nutnost opravy. Díky \emph{Sentry} budu mít informace o~chybě z~\acrshort{FE} i~\acrshort{BE}.
