...
const App = () => {
    return (
        ...
        \section{Webový server}
            \emph{Webový server} se~stará o~odpovědi na požadavky, přicházející většinou na~port \emph{443} nebo \emph{80}. Server dostane požadavek ve~formě \acrdefprint{URL}{Uniform Resource Locator}, někdy \acrdefprint{URI}{Uniform Resource Identifier}, obsahující cestu k~požadovanému zdroji (souboru/službě).
            ...
            \emph{Webové servery} mohou být~např.~\emph{Apache}, \emph{Nginx}, \emph{Lighttpd}, \emph{Caddy} nebo~\emph{IIS}\dots
...
UPDATE users SET email = 'mujmejl@seznam.cz\', role=\'admin', WHERE id = 42'
	...
	\chapter{Návrh aplikace}
        ...
        \section{Výběr technologií}
            ...
            \textbf{Mathjax} (\url{https://www.mathjax.org}), je nástroj, který umožňuje zobrazit na~webu matematické zápisy mmj.~ve~formátu \LaTeX zápisu. Použiji jej~tedy pro~zobrazení matematických výrazů v~textu příkladů.
...
class dataLoader extends Endpoints{
    ...
    \chapter{Závěr}
        ...
        Aplikace \emph{Písemkomat} je~dostupná také pomocí~\emph{API} a~je možné ji~integrovat do~dalších prostředí formou pluginů. Uživatelé tak budou moci používat generované příklady např.~v~prostředí \textbf{Moodlu} nebo~\acrdefprint{CMS}{Content Management System} jako~např.~\textbf{Wordpress}. Dokumentace k~\emph{API} je~k~dispozici~ve~veřejné části webu.
