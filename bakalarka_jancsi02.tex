...
\begin{document}
    ...
    Většina dnešních projektů je~vyvíjena jako~\emph{aplikace}. Tím oddělíme logiku aplikace od~jejího zobrazení, což nám umožňuje vývoj funkční části aplikace nezávisle na~uživatelské části.
    ...
    Na~začátku plánování projektu je~dobré se~zamyslet i~nad~tím, jaké technologie budeme používat a~jakým způsobem budeme aplikaci vyvíjet.

    V~profesionálním prostředí je~potom důležité i~to, jestli budeme aplikaci vyvíjet sami nebo~v~týmu a~zda na~trhu práce je~dostatek lidí, kteří dané technologie znají.
	...
	\section{Návrh aplikace}
        ...
        \subsection{Výběr technologií}
            ...
            Navíc \emph{React Hooks} jsou stále ve~vývoji a~mohou se~změnit.
            ...
            Pro vytvoření jednoduché komponenty typu \texttt{H1} s~třídou `greeting` a~nápisem \texttt{Hello World!} v~Reactu můžeme použít např.~tento kód:
            \begin{minted}{js}
                function ComponentA({ name }) {
                    return React.createElement(
                        'h1', // typ prvku
                        { className: 'greeting' }, // atributy
                        'Hello World!' // obsah
                    );
                }
            \end{minted}

            Komponenty mohou obsahovat i~další komponenty. Tomu říkáme \emph{zanořování} nebo anglicky \emph{nesting}. To~umožňuje vytvářet složitější komponenty s~více prvky.
            Při použití \href{https://react.dev/reference/react/createElement}{\texttt{React.createElement} (viz. dokumentace)} by~takový kód byl složitě čitelný a~i~proto máme k~dispozici \textbf{JSX}.
            ...
            Použití komponent za~použití \emph{JSX} může vypadat třeba takto:
            \begin{minted}{jsx}
                const App = () => {
                    return (
