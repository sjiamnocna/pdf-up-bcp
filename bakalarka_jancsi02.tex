...
UPDATE users SET email = 'mujmejl@seznam.cz\', role=\'admin', role=\'admin\' WHERE id = 42
	...
	\section{Návrh aplikace}
        ...
        Aplikace \emph{Písemkomat}, poskytne registrovaným uživatelům možnost vytvořit vytvořit si příklady, umístěné ve~volitelně strukturovaných kategorií - podle předmětu, ročníku, tématu... Tyto příklady bude možné následně využít při tvorbě testů, kde~bude mít~uživatel možnost zvolit kategorie, ze~kterých se~vybere určené množství příkladů, nebo konkrétní příklady, které se~mají do~písemky vložit. Uživatelé budou mít možnost přihlášení pomocí více způsobů, vč.~externích poskytovatelů ověření identity a~samozřejmě také pomocí přihlašovacího jména a~hesla.
        ...
        Proměnné se~při~generování výsledné podoby příkladu nahradí vygenerovanými čísly, výrazy nebo funkčními prvky. Pokud budou vygenerovány příklady, uloží se~do~databáze konkrétní hodnoty proměnných a~vygenerovaných odpovědí, aby bylo možné identifikovat přesnou verzi příkladu. Výsledná podoba příkladu jednoduchého výrazu může vypadat např.~takto:
        \begin{align*}
        ...
        \subsection{Výběr technologií}
            ...
            A~\emph{Alpine Linux} je~odlehčenou distribucí Linuxu, která je~určena zejména pro~nasazení v~kontejnerech, kde~očekáváme malou zátěž na~uložiště a~zdroje, nicméně jde~o~plnohodnotný systém a~je~možné ji~použít i~jako~samostatný systém. Celková velikost distribuce po~instalaci je~pouze 5~MB, což je~velice málo.
