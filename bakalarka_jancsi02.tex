...
class dataLoader extends Endpoints{
	...
	\section{Postup implementace}
        ...
        \subsubsection{Generátor}
            ...
            V~matematickém režimu, který bude~zapnutý nebo~vypnutý podle nastavené kategorie a~konkrétního příkladu, resp.~proměnné, je~potřeba~vyčíslit hodnotu výrazu zapsanou v~\emph{tokenech}, např.~\texttt{randInt(0,42) + 1} pro~náhodné liché číslo od~1 do~43. To~realizuju pomocí algoritmu podobnému tzv.~\textbf{Postfixové notaci} (\textbf{Reverzní polská notace}), která se~k~vyčíslování výrazů v~počítačových programech běžně používá. \cite{postfixnotation}
	...
	\section{Závěr}
        ...
        Základ pro~tvorbu aplikace pomocí jazyka \emph{Go} a~knihovny \emph{Vite} je~k~dispozici na~\emph{Gitlabu} \href{https://gitlab.com/sjiamnocna/gormrest}{\texttt{sjiamnocna/gormrest}}. Dokumentace ke~kódu je~v~komentářích, dokumentačních řetězcích a~souborech typu \textbf{Markdown} v~popisu repozitářů.
