...
UPDATE users SET email = 'mujmejl@seznam.cz\', role=\'admin', WHERE id = 42'
	...
	\section{Návrh aplikace}
        ...
        \subsection{Diagram případů užití}
            ...
            \begin{itemize}
                \item \harddata{Neregistrovaný uživatel} může pouze prohlížet vygenerované příklady a~testy ze~sdíleného odkazu, případně volně dostupné příklady a~testy ostatních uživatelů. Může procházet veřejnou část aplikace, vybrat si~jazyk aplikace, zaregistrovat~se ap.

                \item \harddata{Uživatel}, který se~zaregistroval do~aplikace~--~může vytvářet a~spravovat kategorie, příklady a~testy. Může si~měnit osobní údaje a~přidávat způsoby přihlášení, případně udělit právo ke~spolupráci na~příkladech.
        
                \item \harddata{Administrátor} může kontrolovat a~zasahovat do~dění v~celé aplikaci, pokud je~to~potřeba~--~měnit uživatele a~všechen další obsah. Bude mít na starosti údržbu systému.

                \item \harddata{Systém} je fiktivní aktér, který provádí pravidelné kontroly a~další akce.
            \end{itemize}            
        ...
        \subsection{Návrh struktury databáze}
            ...
            O~problematiku tvorby databázových tabulek se~postará \emph{ORM}, který sám vytvoří tabulky z~modelů dat, které bude aplikace používat. Autor aplikace se~tak nemusí starat o~tvorbu tabulek a~jejich vztahů, ale~může se~soustředit na~tvorbu aplikace, nicméně by~měl pamatovat na~zásady indexování a~optimalizace databáze. Tyto zásady jsou popsány jako 1\,--\,3.~normální formy a~do zásad \emph{ACID} (Atomicity, Consistency, Isolation, Durability). \cite{interval:normalniformydb, bmcACIDExplained}
...
class dataLoader extends Endpoints{
	...
	\section{Postup implementace}
        ...
        \subsubsection{Generátor}
            ...
            \emph{Parser} musí rozložit zápisy hodnot proměnných na~jednotlivé \emph{dílčí řetězce} (\emph{tokeny}) a~identifikovat jejich typ. K~tomu použiju modul \href{github.com/bzick/tokenizer}{\texttt{bzick/tokenizer}}. Ten~identifikuje základní typy~--~číslo, identifikátory a vyfiltruje tzv.~\textbf{bílé znaky} (mezery, ukončení řádku~ap.) a~poskytne mi~seznam řetězců (\texttt{keyword}). Podle kontextu (typů okolních \emph{tokenů}) je~potřeba sloučit a~určit, jak je~bude potřeba zpracovávat. Písmenko za~číslem bude polynom, identifikátor se~závorkama je~funkce, identifikátor se~znakem \texttt{\$} je~použitá proměnná.
            
            Jako první musí proběhnout spuštění funkcí ve~všech proměnných. Tím~jsou vygenerovány náhodné nebo vypočítané hodnoty. Proměnné, které budou označeny za~kompletní (tj.~nebudou obsahovat žádné funkce ani~proměnné) a~nebudou dál zpracovávány. Budou použity pro~tvorbu cache, aby bylo možné je~při~dalším použití pouze nahradit jejich hodnotou bez~rozsáhlých výpočtů.

            Poté je možné provést nahrazení proměnných, spojením \emph{tokenů} do~jednoho řetězce a~nahrazením proměnných jejich hodnotami. Tuto akci je nutné opakovat vícekrát, dokud nejsou vyřízeny všechny závislosti na~proměnných. V~případě, že~se~nepodaří vyřešit všechny závislosti, jde~o~chybu zadání a~přeruším generování příkladu.

            Pro~vyčíslení koeficientů jednotlivých členů výrazu (pokud to uživatel u~proměnné nezakáže) pak nejdříve dosadím hodnoty existujících proměnných, spustím funkce a~poté pomocí tzv.~\textbf{Postfixové notace} (Někdy také \textbf{Reverzní polská notace}), která se~k~vyčíslování výrazů pomocí počítačů běžně používá, vyčíslím koeficienty u~případných mnohočlenů. \cite{postfixnotation}

            Protože pracuju s~jazykem Go, obdrží \emph{generátor} data ve~formě \emph{struktury} definice příkladu a~výsledkem bude struktura obsahující zadání a~případně vygenerované možnosti s~nahrazenými a~vyčíslenými hodnotami, aby bylo možné hodnoty předat zobrazení (generátoru \emph{PDF} nebo \emph{JSON} pro~použití na~webu). Výsledná data budou obsahovat pouze zadání otázky a~možnosti odpovědí. Na~místě zobrazení uživatelského vstupu bude vložen \emph{placeholder}, který bude zpracován až~podle~volby~zobrazení. Pokud bude generátor vytvářet nějaké konkrétní hodnoty (jako náh. čísla nebo výpočty nebo řazení odpovědí), výsledek bude obsahovat kladný \emph{příznak}, že~ke~generování došlo. \emph{Generátor} samotný nemá přístup k~databázi, takže uložení vygenerovaných hodnot zajistí webserver, hodnoty si~načte ze~struktury obdržené z~generátoru. Potom konkrétní hodnoty uloží do~databáze a~obdrží jednoznačný identifikátor z~generovaných hodnot pro~identifikaci příkladu a~odpovědí.
        ...
        \subsubsection{Autentizace uživatelů}
            ...
            Na~\emph{Frontendu} budu používat nástroj \emph{Firebase}, což~je~nástroj od~\emph{Google}, která umožňuje mmj.~jednoduchou autentizaci uživatelů pomocí různých webových služeb. Po~přihlášení dostane každý takový uživatel svůj unikátní \emph{UID}
        ...
        \subsection{Frontend}
            ...
            \textbf{UI (User Interface)}~--~uživatelské rozhrání bude co~nejjednoduší, využiju k~tvorbě knihovnu \emph{Semantic UI}, která nabízí jednoduché a~přehledné komponenty, které jsou navíc přizpůsobitelné. \emph{Semantic UI} je~dostupná jako \emph{npm} balíček, který je~možné nainstalovat pomocí \emph{npm} nebo~\emph{yarn}.
	...
	\section{Závěr}
        ...
        Výsledná aplikace je veřejně dostupná na~adrese \href{https://pisemkomat.cz}{\texttt{pisemkomat.cz}}. Kód některých částí této práce je~možné veřejně najít na~\href{https://gitlab.com/pisemkomat}{Gitlabu}. \emph{Generátor příkladů} a~\emph{parser} bude vydán jako Open-source.
