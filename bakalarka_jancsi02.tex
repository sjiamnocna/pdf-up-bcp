...
UPDATE users SET email = 'mujmejl@seznam.cz\', role=\'admin', WHERE id = 42'
	...
	\section{Návrh aplikace}
        ...
        \subsection{Účel a~popis fungování vyvíjené aplikace}
            ...
            Po~nasdílení trvalého odkazu na~daný~příklad/test bude umožněno zobrazit náhodně vygenerovanou variantu příkladu, případně si~příklad přidat a~upravit ve~vlastním účtu.
...
r.POST("/age", func(c *gin.Context) {
    ...
    c.JSON(http.StatusOK, gin.H{
        "age": age.Age,
                    ...
                    \end{minted}
                    \caption{Načtení dat z POST požadavku v jazyce Go}
                \end{subfigure}
                \hfill
                \begin{subfigure}[b]{0.45\linewidth}
                    \begin{minted}{php}
...
class dataLoader extends Endpoints{
    ...
    public function actionEndpoint{
        ...
        if ($age < 0){
            ...
            \emph{Alpine Linux} je~odlehčená distribuce \emph{Linuxu}, určená zejména pro~nasazení v~kontejnerech, kde~očekáváme malou zátěž na~uložiště a~zdroje, nicméně jde~o~plnohodnotný systém a~je~možné jej~použít jako hlavní systém. Celková velikost distribuce po~instalaci je~pouze 5~MB, což je~velice málo.
	...
	\section{Postup implementace}
        ...
        Při~vývojí ve~\emph{Vite} využiju nastavení \emph{proxy}, čímž~požadavky, jejichž \emph{query} začíná \texttt{'/api/...'} přesměruju na~kontejner s \emph{backendem} aplikace, pomocí konfigurace~\emph{proxy} ve~\emph{Vite}.
        ...
        \subsection{Backend}
            ...
            Přihlašovací údaje budu tzv.~\textbf{cachovat} [kešovat], což~znamená uložím si~data posledních $n$~uživatelů z~databáze do~aktivní paměti běžícího programu pro~pozdější využití, aby byla rychleji k~nalezení bez~nutnosti přístupu k~databázi nebo~provádění náročných výpočtů, které by~vedly ke~stejným výsledkům. \emph{Cachovat} budu i~části příkladů, které nevyžadují generování dat, pouze jednoduché nahrazení.
