...
\begin{document}
    ...
    \chapter{Testy, testové úlohy}
         V~této kapitole se~budu zabývat problematikou a~náležitostmi testových úloh a~jaké~možnosti nabízejí moderní technologie ke~zjednodušení jejich tvorby. Zaměřuji se~na~nástroje, které jsou dostupné bezplatně nebo~jsou dostupné s~\textbf{otevřeným zdrojovým kódem} (\textbf{open--source}).
        ...
        \section{Tvorba testů}
            ...
            \uv{\emph{Otázky} by~měly být~pestré, pokrývající co~nejširší spektrum prověřovaného učiva, a~to z~různých úhlů pohledu. Proto je~nutné použít \emph{otázky}, které zjišťují schopnost žáka pracovat s~fakty (značky a~jednotky fyzikálních veličin, matematické vyjádření fyzikálních zákonů~atd.)}.

            Zároveň je~potřeba myslet na~to, že~žáci mají různé schopnosti a~znalosti a~ne pro všechny je~test stejně náročný. Proto je~potřeba k~dosažení spolehlivého ověření znalostí kombinovat testy s~jinými metodami ověřování znalostí a~dávat důraz na~získané schopnosti a~jejich aplikaci, nikoliv na~to, co~si~žák dokázal doslovně zapamatovat. Důležitá je i~práce s~chybou a~individuální přístup k~žákům. \cite{Suchoradsky:testy, chraska:testy, Berkley2017LearningFromErrors}
    ...
    \chapter{Technologie a principy vývoje webových aplikací}
        ...
        \section{HTML s dynamickým obsahem}
            HTML stránky mohou mít i~dynamický obsah. To~znamená, že~se~obsah může~měnit podle toho, jak se~stránkou uživatel pracuje. Na~úrovni serveru se~může generovat \emph{HTML} stránka, která obsahuje dynamický obsah. V~dnešní době je~oblíbený přístup generovat výslednou stránku na~serveru a~pomocí \emph{javascriptu} si~při~změnách od~\emph{serveru} vyžádat pouze data, nicméně v minulosti se~dynamický obsah generoval převážně na~serveru pomocí technologií jako~\emph{PHP (HyperText Preprocessor)}, \emph{ASP.NET}, které na~serveru vygenerovaly kompletní \emph{HTML kód} stránky i~s~načtenými daty.
        ...
        \section{Kompilátor a interpret}
            ...
            \emph{Kompilátor} při~překladu provádí optimalizace, které zvyšují výkon a~snižují nároky programu na~\emph{hardware}.

            Takový program je~rychlejší, nicméně při~jeho tvorbě je~potřeba řešit některé problémy, např.~správu paměti. Jde např.~o~jazyky \textbf{C a C++}, \textbf{Rust} nebo~\textbf{Go}.

            \textbf{Interpretovaný} jazyk je~naopak vykonáván programem, kterému říkáme interpret. Ten~čte~příkazy a~přímo je~vykonává. To~znamená, že~při~každém spuštění se~příkazy překládají znovu. \textbf{Interpret} je~program, který musí běžet, zatímco příkazy interpretuje, což~je~také jeden z~důvodů, proč bývají interpretované jazyky pomalejší a~náročnější na~využítí zdrojů, než~ty~kompilované.
            
            Interpretování ale přináší výhody, zejména pro~vývojáře. Například je~možné příkazy vykonávat ihned, bez~nutnosti překladu a~není potřeba obstarávat správu paměti. Do~této kategorie patří např.~\textbf{JavaScript}, \textbf{PHP}, \textbf{Python} nebo~\textbf{Perl}~\cite{ueda:compiled}.
        ...
        \section{Překladač (kompilátor)}
            ...
            \begin{description}
                ...
                \item[Syntaktická analýza] kontroluje, jestli jsou jednotlivé \emph{tokeny} správně strukturovány a~v~jakém vztahu k~sobě stojí. Generuje \emph{abstraktní syntaktický strom (AST = Abstract Syntax Tree)}, který reprezentuje strukturu programu.
                ...
                \item[Optimalizace] v~této fázi optimalizujeme kód, aby byl ve~výsledku rychlejší a~méně náročný na~procesor. Optimalizace se~děje v~závislosti na~použitém jazyce a~překladači a~může být~prováděna několikrát v~různých fázích překladu (a~nemusí být prováděna vůbec).
                \item[Generování kódu] Když~máme \emph{AST}, můžeme generovat výslednou podobu programu nebo dat v~požadované formě~--~binární soubor programu, optimalizovaný bytecode pro spouštění pomocí interpretu, program zapsaný v~jiném jazyce nebo interpretujeme jako~jinou formu dat.
            \end{description} \cite{baeldungCompilersWork, compilers}
        ...
        \section{Procedurální a~objektově orientované programování (OOP)}
            ...
            \begin{description}
                ...
                \item[Abstrakce] nemusí být~implementována funkcionalita tříd a~nelze je~samostatně \emph{inicializovat}. To~umožňuje vytvářet obecné (tzv.\emph{bázové})~třídy, které mohou být použity jako vzor pro~odvození jiných konkrétních tříd, využívajících jejich vlastnosti a~metody.
            \end{description}
...
func main() {
	...
	animalMakeSound(b)
        ...
        \section{React}
            ...
            V~této práci nicméně pro~správu stavu aplikace využiju knihovnu \textbf{Zustand}, která je~moderní alternativou ke~\emph{Context API} a~nabízí spoustu výhod oproti \emph{Contextu} i~knihovnám jako~\textbf{Redux}, \textbf{Mobx}~apod.~\cite{openreplayReactState}.
    ...
    return (
        ...
        \section{Zabezpečení}
            ...
            \subsection{JWT~--~JSON Web Token}
                ...
                \begin{description}
                    \item[Hlavička (header)}, která obsahuje informace o~typu tokenu a~typu šifrování podpisu.
                    \item[Tělo tokenu (body)] obsahuje data, která chceme uchovat (\emph{payload}).
                ...
                Jednou z~výhod \emph{JWT} je~možnost přenositelnosti. Token může být~vytvořen na~jednom serveru a~ověřen na~jiném, který má k~dispozici daný klíč (\cite{ieee:jwt}). Můžeme tak např.~umožnit jednotné přihlášení v~rámci několika služeb. \emph{JWT}~není vázán na~symetrické šifrování a~kvůli přenositelnosti se~často využívá \emph{asymetrické šifrování}, kde hlavní server má~tzv.~\textbf{privátní klíč} a~ostatní mají svůj~vlastní \textbf{veřejný klíč}, odvozený od~privátního, kterým ověřují, že~byl~token vytvořen na~správném serveru~\cite{miguelgrinbergJSONTokens}.
                
                Díky \emph{asymetrickému šifrování} můžeme vytvořit více \emph{veřejných klíčů} a~přidělit je~různým službám, které je~pak mohou používat pro~ověření \emph{tokenu} z~původního \emph{serveru}.
...
UPDATE users SET email = 'mujmejl@seznam.cz\', role=\'admin', WHERE id = 42'
        ...
        \section{I18n~--~Mezinárodní lokalizace}
            \emph{I18n} (\textbf{Internationalization}) je~proces přizpůsobení aplikace pro~daný jazyk a~kulturu, kromě překladu např.~formát data, měny nebo jednotky. Standard \emph{I18n} zahrnuje i~různé formy slov, např.~pro~různá množství~\cite{w3Internationalization}.

            Každý znak je~v~počítači reprezentován číslem. V~minulosti používaly počítače k~ukládání znaků kódování \emph{ASCII}, které (z~historických důvodů) obsahovalo pouze 128~znaků (7 bitů, tj.~rozsah 0\,--\,$2^7 - 1$ (127)). To~znamená, že~bylo možné zobrazit pouze anglické texty a~některé interpunkční znaménka. 
        ...
        \section{UML~--~Unified Modeling Language}
            ...
            \subsection{Class Diagram}
                ...
                \begin{description}
                    ...
                    \item[Příklad] značí konkrétní úlohu, kterou vkládáme do~testů. Obsahuje název, text, typ (otevřený, uzavřený), odpovědi, proměnné, odkaz na kategorii a~pokyny pro~generování výsledku.
	...
	\chapter{Návrh aplikace}
        ...
        Na~začátku plánování projektu je~dobré se~zamyslet i~nad~tím, jaké technologie budeme používat, jak budou jednotlivé součásti komunikovat a~jakým způsobem je~aplikace vyvíjena. V~profesionálním prostředí je~potom důležité přemýšlet i~nad~tím, jestli budeme aplikaci vyvíjet sami nebo~v~týmu a~zda je~na~trhu práce dostatek lidí, kteří dané technologie znají~\cite{bctynovsky:specifikacepozadavku}.
        ...
        \section{Případ použití aplikace}
            ...
            \begin{align*}
                \text{Vyřeš v oboru \mathbb{N}:} \quad \$Ax + \$B &= \$C + \$D \\
                \begin{aligned}
            ...
            \begin{align*}
                \text{Vyřeš v oboru \mathbb{N}:} \quad 3x + 5 = 4 + 9 \\
                \begin{aligned}
                    a) \quad &\text{Nemá řešení v oboru }\mathbb{N} \\
                    b) \quad &\text{x = 3} \\
                    c) \quad &\text{x = 5} \\
	...
	\chapter{Postup implementace}
        ...
        \subsection{Funkce Main}
            ...
            \begin{description}
                ...
                \item[/api/cat]~--~\emph{CRUD} akce spojené s~kategoriemi. Kategorie jsou~určeny pro~zařazení úkolů a~testů, ale také pro~identifikaci předmětů, které učitel učí.
        ...
        \section{Frontend}
            ...
            Důležitým bodem při~vývoji je~neukládat citlivé údaje, jako např.~klíče nebo~soukromé údaje v~kódu (tzv.~\textbf{hardcoded}), kde k~nim bude mít~přístup téměř kdokoliv a~vystavujeme se~tak~riziku zneužití údajů. Proto jsou takové údaje načítány z~\emph{proměnných prostředí}, které budou uloženy v~souboru \texttt{.env} a~\emph{Docker} si~je~automaticky načte. Jelikož \emph{NextJS} neumožňuje načtení \emph{env proměnných} do~veřejné části aplikace, budu je~při spuštění \emph{NextJS} aplikace pomocí skriptu z~prostředí načítat do~\emph{Javascriptového} souboru a~dále s~nimi budu pracovat jako s~\emph{konstantami}. \emph{Backend} napsaný v~\emph{Go} umí~takové proměnné číst pomocí \texttt{os.Getenv("nazevpromenne")}.
...
const f = fs.open('env_vars.js', 'w', function(err, fd) {
    ...
    fs.close(fd, function() {
        console.log('wrote the file successfully');
                ...
                \end{minted}
                \caption{Skript \emph{Node.js} načítající \emph{env proměnné} do~\emph{Javascriptového} souboru.}
                \label{oop:difference}
            \end{figure}
            ...
            \textbf{UI (User Interface)} (\textbf{uživatelské rozhrání}) bude~vytvořeno s~použitím \textbf{Tailwind CSS}. \emph{Tailwind} je~\uv{\textbf{utility-first}} knihovna, tj. že~se soustředí na~poskytnutí jednoduchých prvků, které jsou připraveny jako~CSS třídy, a~ke~komponentám je~možné je~přidávat jako třídy, které upraví jejich vzhled. Skládáním těchto tříd je~možné vytvořit vzhled, který potřebujeme. \emph{Tailwind} je~také možné rozšířit o~vlastní třídy nebo~další knihovny, které rozšiřují jeho funkčnost a~mohou být~použity v~rámci celé aplikace~\cite{Rappin2022ModernCSSTailwind}.
    ...
    \chapter{Závěr}
        ...
        Bohužel jsem po~mnoha pokusech s~pluginy a~konfiguracemi pro~\emph{I18n} zjistil, že~je~v~\emph{NextJS} již~od~r.~2021 nahlášena chyba (viz.~\href{https://github.com/vercel/next.js/issues/21565}{odkaz}), která znemožňuje použití \emph{I18n} a~konfigurace \emph{proxy}. Z~tohoto důvodu bude řešení problému realizováno mimo její rámec.
